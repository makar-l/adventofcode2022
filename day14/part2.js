...
const getSandCount = data => {
    ...
    const xOffset = minX - floorOffset;

    const rocksWithOffset = rocks.map(line => line.map(([x, y]) => [x - xOffset, y]));

    const caveWidth = maxX - minX + 2 * floorOffset + 1;
    const caveHeight = maxY + 2;

    const cave = new Array(caveHeight).fill(null).map(() => new Array(caveWidth).fill('.'));

    cave.push(new Array(caveWidth).fill('#'));
    ...
    while (!stop) {
        ...
        if (sandCoords !== null) {
            ...
            if (sandX === startX - xOffset && sandY === 0) {
                stop = true;
            }
        } else {
            throw 'short floor';
